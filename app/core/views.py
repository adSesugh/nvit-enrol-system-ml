...
def nin_update():
    ...
    students = Student.query.with_entities(Student.id, Student.student_no, Student.phone_number, Student.first_name, Student.middle_name, Student.last_name, Student.lga_of_origin, Student.headshot, Student.means_of_id_no, Student.gender, Student.record_sealed).filter(
        (Student.record_sealed is not False) & (Student.record_sealed is not True)).order_by(Student.id).all()
